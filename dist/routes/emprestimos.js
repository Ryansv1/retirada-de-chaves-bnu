...
export default async function Emprestimos(app) {
    ...
    app.withTypeProvider().route({
        ...
        handler: async (req, reply) => {
            ...
            try {
                ...
                app.log.fatal(error);
                throw new HTTPError(500, 'Erro interno do servidor ao processar a solicitação de empréstimo');
            }
        },
    });
    app.withTypeProvider().route({
            ...
            try {
                ...
                await database.$transaction(async (tx) => {
                    ...
                    if (!emprestimoPendente) {
                        throw new HTTPError(404, 'Não há empréstimo vigente para esta chave.');
                    }
    ...
    app.withTypeProvider().route({
        ...
        schema: {
            body: z
                ...
                if (data.status === 'DEVOLVIDO' && !data.dataRetorno)
                    return false;
                if (data.status === 'PENDENTE' && data.dataRetorno)
                    return false; // Regra extra: não pode ter data de retorno se está pendente
                return true;
            }, {
        ...
        preHandler: [AuthenticatedOnly],
        handler: async (req, reply) => {
            const { chaveId, dataRetirada, justificativa, status, dataRetorno } = req.body;
            try {
                const novoEmprestimoAdministrativo = await database.$transaction(async (tx) => {
                    const chave = await tx.chave.findUnique({
                        where: {
                            id: chaveId,
                        },
                    ...
                    if (status === 'PENDENTE') {
                        ...
                        if (emprestimoExistente) {
                            throw new HTTPError(400, 'Já existe um empréstimo PENDENTE para essa chave. Não é permitido criar mais um empréstimo para a chave');
                        }
                    }
                    const novoEmprestimo = await tx.emprestimo.create({
                        data: {
                ...
                app.log.fatal(error);
                throw new HTTPError(500, 'Erro interno do servidor ao criar empréstimo administrativo');
            }
        },
    });
}
//# sourceMappingURL=emprestimos.js.map