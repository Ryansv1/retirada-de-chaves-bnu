...
export default async function Usuarios(app) {
            ...
            try {
                const resultado = await database.$transaction(async (tx) => {
                    ...
                    if (chavesEncontradas.length !== chavesId.length) {
                        throw new HTTPError(404, 'Uma ou mais chaves não foram encontradas.');
                    }
                    // 3. Lógica para criar APENAS as novas autorizações (a parte mais importante)
                    const autorizacoesAtuais = new Set(usuario.Chaves_Usuarios.map((auth) => auth.chaveId));
                    const novasChavesParaAutorizar = chavesId.filter((id) => !autorizacoesAtuais.has(id));
                    ...
                    if (novasChavesParaAutorizar.length === 0) {
                        return {
                            message: 'Nenhuma nova autorização necessária. O usuário já possui permissão para todas as chaves informadas.',
                        };
                    }
                ...
                app.log.fatal(error);
                throw new HTTPError(500, 'Erro interno do servidor ao processar nova autorização');
            }
        },
    });
    app.withTypeProvider().route({
        ...
        handler: async (req, reply) => {
            ...
            try {
                ...
                app.log.fatal(error);
                throw new HTTPError(500, 'Erro interno do servidor ao remover autorização');
            }
        },
    });
    app.withTypeProvider().route({
        ...
        handler: async (req, reply) => {
            ...
            try {
                ...
                app.log.fatal(error);
                throw new HTTPError(500, 'Ocorreu um erro interno ao listar os usuários.');
            }
        },
    });
}
